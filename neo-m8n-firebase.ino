...
// GPS Module connections (NEO-M8N compatible)
...
void loop() {
  ...
  // Improved GPS data reading for NEO-M8N
  ...
  // Extended GPS detection timeout for NEO-M8N cold start
  if (millis() > 10000 && gps.charsProcessed() < 10) {
    Serial.println("No GPS detected - check wiring or antenna");
  }
}
...
void processAndSendGPSData() {
  ...
  if (gps.location.isValid() && gps.location.age() < 2000) {  // Check data freshness
    ...
    if (gps.date.isValid() && gps.time.isValid()) {
      timestamp = String(gps.date.year()) + "-" + 
               ...
               String(gps.date.day() < 10 ? "0" : "") + String(gps.date.day()) + " " +
               String(gps.time.hour() < 10 ? "0" : "") + String(gps.time.hour()) + ":" +
